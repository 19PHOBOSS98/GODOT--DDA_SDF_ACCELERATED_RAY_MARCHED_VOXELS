...
const RECORD_KEY = KEY_C # Change this to the key you want to start/stop recording the frames.
const SAVE_KEY = KEY_Z # Change this to the key you want to save the frames recorded.
...
func save_frames(userdata):
	...
	for image in _images:
		...
		image.save_png(str(output_folder) + "/" + "%03d" % i + ".png")
		i+=1
	_label.text = "Done! Check the "+output_folder+" folder"
	_images.clear()

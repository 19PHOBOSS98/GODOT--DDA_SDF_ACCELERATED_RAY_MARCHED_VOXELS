...
const int BOUNCE = 2; //light bounce count
...
void DDA_SDF_IntersectVoxels(vec3 ray_origin, vec3 ray_direction, inout vec3 bestHit_position,inout float bestHit_distance,inout vec3 bestHit_normal,inout vec3 bestHit_albedo, inout vec3 bestHit_specular,inout float bestHit_emission, vec3 voxel_specular,float voxel_emission)
{
	...
	vec3 rayUnitStepSize = vec3( sqrt( 1f+(ray_direction.y/ray_direction.x)*(ray_direction.y/ray_direction.x) ) , sqrt( 1f+(ray_direction.x/ray_direction.y)*(ray_direction.x/ray_direction.y) ),sqrt( 1f+(ray_direction.x/ray_direction.z)*(ray_direction.x/ray_direction.z) ) );
	vec3 rayLen1D;
	ivec3 steps;

	steps.x = ray_direction.x<0f?-1:1;
	steps.y = ray_direction.y<0f?-1:1;
	steps.z = ray_direction.z<0f?-1:1;
	vec3 push = vec3(steps)*0.1;
	
	ivec3 mapCheck = ivec3( int(floor(cellRayOrigin.x+push.x)), int(ceil(cellRayOrigin.y+push.y)),int(floor(cellRayOrigin.z+push.z)) );    //move
	//vec3 mapCheck = vec3( floor(cellRayOrigin.x+push.x), ceil(cellRayOrigin.y+push.y),ceil(cellRayOrigin.z+push.z) );    //z axis is backwards in godot

	bool isInMap = (mapCheck.x >= 0 && float(mapCheck.x) < mapSize.x && mapCheck.y >= 0 && float(mapCheck.z) < mapSize.z && mapCheck.z >= 0 && float(mapCheck.z) < mapSize.z);
	//float boxState = isInMap ? simplexNoiseController(3,0.5,2,1, mapCheck):0f;  //boxState: 2: solid, 1:surface, 0:air
	float boxState = isInMap ? getBox(mapCheck):0f;  //boxState: 2: solid, 1:surface, 0:air
	rayLen1D.x = ( (float(mapCheck.x)+( 1f+float(steps.x)*0.5 ) ) - cellRayOrigin.x ) *float(steps.x) *rayUnitStepSize.x;     //draw
	rayLen1D.y = ( (float(mapCheck.y)+( 1f+float(steps.y)*0.5 ) ) - cellRayOrigin.y ) *float(steps.y) *rayUnitStepSize.y;
	rayLen1D.z = ( (float(mapCheck.z)+( 1f+float(steps.z)*0.5 ) ) - cellRayOrigin.z ) *float(steps.z) *rayUnitStepSize.z;

	float dist = 0f;
	
	vec3 intersection;
	
	while(boxState != 2f && dist<maxDist){
		
		bool walk_x = rayLen1D.x<rayLen1D.y && rayLen1D.y<rayLen1D.z;  //compare
		bool walk_y = rayLen1D.y<rayLen1D.z;	//compare
		dist = walk_x?rayLen1D.x:(walk_y?rayLen1D.y:rayLen1D.z);
		
		intersection = cellRayOrigin + ray_direction *dist;
		//mapCheck = vec3(floor(intersection.x+push.x),ceil(intersection.y+push.y),floor(intersection.z+push.z));  //move
		
		mapCheck.x += walk_x ? steps.x:0;
		mapCheck.y += walk_x ? 0:(walk_y?steps.y:0);
		mapCheck.y += walk_x ? 0:(walk_y?0:steps.z);
		
		isInMap = (mapCheck.x >= 0 && float(mapCheck.x) < mapSize.x && mapCheck.y >= 0 && float(mapCheck.z) < mapSize.z && mapCheck.z >= 0 && float(mapCheck.z) < mapSize.z);
		//boxState = isInMap ? simplexNoiseController(3,0.5,2,1, mapCheck):0f;  //boxState: 2: solid, 1:surface, 0:air
		boxState = isInMap ? getBox(mapCheck):0f;  //boxState: 2: solid, 1:surface, 0:air
		 if(boxState==2f){
			vec3 vCenter = vec3(mapCheck) + vec3(0.5,-0.5,0.5); ////voxIndex is always located at the top left (backtopleft) corner of a grid box; to get to the center move the index down, right and forward; voxel dimensions are isometric
			bestHit_normal = getVoxelNormal(intersection, vCenter);
			bestHit_position = intersection*cellSize;
			bestHit_distance = dist*cellSize;
			//bestHit_albedo = vCenter;
			bestHit_albedo = vec3(0f,1f,0f);
			bestHit_specular = voxel_specular;
			bestHit_emission = voxel_emission;
			return;
		}
/*
		//float skip = 2f-boxState;	
		rayLen1D.x += walk_x ? (rayUnitStepSize.x*skip) : 0f;
		rayLen1D.y += walk_x ? 0f:(walk_y? rayUnitStepSize.y*skip:0f);
		rayLen1D.z += walk_x ? 0f:(walk_y? 0f:rayUnitStepSize.z*skip);
*/
		rayLen1D.x += walk_x ? (rayUnitStepSize.x) : 0f;
		rayLen1D.y += walk_x ? 0f:(walk_y? rayUnitStepSize.y:0f);
		rayLen1D.z += walk_x ? 0f:(walk_y? 0f:rayUnitStepSize.z);
	}	
}
...
*/
	...
	DDA_SDF_IntersectVoxels(ray_origin, ray_direction, bestHit_position, bestHit_distance, bestHit_normal, bestHit_albedo, bestHit_specular,bestHit_emission, vec3(0f, 0f, 0f),0f);
